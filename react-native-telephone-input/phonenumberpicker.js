...
class PhoneNumberPicker extends React.Component {
  ...
  getCountryFromCCA2(cca2) {
    ...
    if (countryName.length == 0 || cca2.length == 0 || callingCode.length == 0) {
      countryName = this.state.country.callingCode.length
      > 0 ? 'Invalid country code' : 'Choose a country';
  ...
  CallingCodeChanged(updatedCallingCode) {
    let countryFromCallingCode = this.getCountryFromCallingCode(updatedCallingCode,
      this.state.phoneNo);
